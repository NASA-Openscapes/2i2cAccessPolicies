...
theme_set(
  ...
  theme(
    ...
    plot.subtitle = element_textbox_simple(margin = unit(c(5,5,5,5), "pt")),
    plot.caption = element_textbox_simple(halign = 1, margin = unit(c(5,0,0,0), "pt"))
    )
)
```
...
total_monthly_cost_plot <- ggplot(total_monthly_usage_costs) +
  ...
  labs(
    title = glue::glue(
      "The total cost of all AWS Services for running the NASA Openscapes 2i2c",
...
monthly_cost_by_service_plot <- ggplot(
  ...
  aes(x = start_date, y = cost, fill = service)
) +
  geom_col() +
...
[Access is managed via GitHub](https://openscapes.cloud/github-access.html) by
assigning user's GitHub usernames to specific
teams.

The "workshop" namespace is used specifically for large workshops and access
is granted on the day of the workshop by use of a
[shared password](https://openscapes.cloud/password-access.html) rather
